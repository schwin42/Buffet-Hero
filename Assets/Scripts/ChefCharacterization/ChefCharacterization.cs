...
//		}
	...
	private void GenerateAnswers ()
	{
		...
		FoodLogic.ShuffleList (this.Answers);
	}
	...
	private FoodAttribute GetUniqueAttribute (List<FoodAttribute> destinationList, AttributeType targetType, string targetTagId = null)
	{
		...
		while (prospectiveAttribute == null || destinationList.Contains(prospectiveAttribute)) {
			if (targetTagId != null) {
				...
				prospectiveAttribute = GameData.GetRandomAttributeFromData (targetType, targetTagId);
			} else {
				prospectiveAttribute = GameData.GetRandomAttributeFromData (targetType);
			}
			if (i > 100) {
	...
	public void StartGame () {
		...
		foreach(Tag tag in GameData.Instance.TagData) {
			if(GameData.TagListContainsId(nationalities, tag.Id) || tag.TagType != "nationality") {
				continue;
			}
			nationalities.Add(tag);
		}
		FoodLogic.ShuffleList(nationalities);
