...
//	void Start () {
	...
	public List<float> DeriveRatingPercentiles()
	{
		...
		{
			Food food = GameData.GetRandomFoodFromData();
			foodTrials.Add (food);
...
//		}
		List<Tag> tagsOfQueriedType = GameData.Instance.TagData.Where(t => t.TagType == this.tagTypeQuery).ToList();
		...
		foreach(Tag tag in tagsOfQueriedType) {
			...
			int hits = GameData.Instance.AttributeData.Where(a => GameData.TagListContainsId(a.tags, tag.Id)).Count();
...
//			outputStrings.Add (hits+ " - " + tag.Id);
		...
		foreach(KeyValuePair<string, int> pair in sortedOutputDict) {
			print ("%" + pair.Value / GameData.Instance.TagData.Count);
			outputStrings.Add (pair.Key + " - " + pair.Value + " - " + (((float)pair.Value) / ((float)GameData.Instance.TagData.Count) * 100F).ToString("F2") + "%");
		}
